...
def pwv_climo(prof, station, month=None):
    ...
    pwv_300 = np.atleast_1d(pwv_300)

    if np.any(pwv_300 > sigma_3[1]):
        # Means the PWV value is outside +3 sigma of the distribution
        flag = 3
    elif np.any(pwv_300 < sigma_3[0]):
        # Means the PWV value is outside -3 sigma of the distribution
        flag = -3
    elif np.any(pwv_300 > sigma_2[1]):
        # Means the PWV value is outside +2 sigma of the distribution
        flag = 2
    elif np.any(pwv_300 < sigma_2[0]):
        # Means the PWV value is outside -2 sigma of the distribution
        flag = -2
    elif np.any(pwv_300 > sigma_1[1]):
        # Means the PWV value is outside +1 sigma of the distribution
        flag = 1
    elif np.any(pwv_300 < sigma_1[0]):
        # Means the PWV value is outside -1 sigma of the distribution
        flag = -1
    else:
